...
client.once('ready', async () => {
  ...
  verileriYukle();
  try {
    const sonuc = await robloxGiris();
    if (!sonuc.basarili) {
      console.error('[Roblox] Giriş başarısız:', sonuc.hata);
    }
  } catch (error) {
    console.error('[Roblox] Beklenmeyen hata:', error);
  }
});
  ...
  if (interaction.isModalSubmit()) {
    if (interaction.customId === 'cookie_modal') {
      ...
      return interaction.editReply({
        content: res.basarili ? `OK: ${res.user}` : `FAIL: ${res.hata}`
      });
    }
  }
...
server.listen(AYARLAR.PORT, () => {
  console.log(`HTTP sunucu ${AYARLAR.PORT} portunda`);
});
